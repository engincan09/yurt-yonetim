...
namespace YurtYonetim.Dal.Middleware
{
    ...
    public class CustomHttpContextAccessor : HttpContextAccessor, ICustomHttpContextAccessor
    {
        ...
        public int GetOrganizationSchemaId()
        {
            return int.TryParse(HttpContext.User.Claims.FirstOrDefault(x => x.Type == "organizationSchemaId")?.Value, out int id) ? id : default;
        }
    }
}